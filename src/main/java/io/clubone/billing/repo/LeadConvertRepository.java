...
public class LeadConvertRepository {
    ...
    public UUID insertClientRole(UUID clientRoleTypeId, UUID locationId, UUID createdBy) {
        ...
        jdbc.update("""
                INSERT INTO clients.client_role (
                    client_role_id, client_role_type_id, location_id, is_active, created_on, created_by
                ) VALUES (?, ?, ?, true, ?, ?)
    ...
    public UUID insertContact(UUID orgClientId, UUID clientId, UUID contactLifecycleId, UUID salutationId,
                              ...
                              UUID homeLocationId, UUID accountId, UUID genderId, String dateOfBirth,
                              Boolean consentToContact, Boolean consentToMarketing, Boolean hasOptOutSms, Boolean hasOptOutEmail,
                              UUID createdBy) {
        UUID id = UUID.randomUUID();
        jdbc.update("""
                INSERT INTO crm.contact (
                    ...
                    home_location_id, account_id, gender_id, date_of_birth,
                    consent_to_contact, consent_to_marketing, has_opt_out_sms, has_opt_out_email,
                    created_by
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, id, orgClientId, clientId, contactLifecycleId, salutationId,
                firstName, lastName, fullName != null ? fullName : (firstName + " " + lastName).trim(),
                email, phone,
                homeLocationId, accountId, genderId, dateOfBirth,
                consentToContact != null ? consentToContact : false,
                consentToMarketing, hasOptOutSms != null ? hasOptOutSms : false, hasOptOutEmail != null ? hasOptOutEmail : false,
                createdBy);
        return id;
    }
